...
def test_abigailv1_not_modified():
    """Sealed V1 baseline must remain at 5cdfee1."""
    import subprocess
    result = subprocess.run(
        ["git", "-C", str(Path.home() / "Abigailv1"), "log", "-1", "--format=%H"],
        capture_output=True, text=True, timeout=10
    )
    head = result.stdout.strip()
    assert head.startswith("5cdfee1"), (
        f"~/Abigailv1 must remain at 5cdfee1, got {head!r}"
    )
